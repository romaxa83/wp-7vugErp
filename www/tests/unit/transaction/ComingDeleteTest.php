...
class ComingDeleteTest extends Unit
{
    /**
     * @var \UnitTester
     */
    ...
    {
        $resultatFilling  = $this->transactionHelper->fillTransaction($this->countTransaction,1,'coming');
    ...
    {
        $arrayRow = $this->transactionHelper->getRandomRow('oper_coming');
        ...
        foreach($arrayRow['transactionRow'] as $oneRow){
            ...
            $calculateData = $this->transactionCalculateHelper->calculateData('delete-product-coming',$oneRow['product_id']);

            $updateRow = Yii::$app->db
                ->createCommand("SELECT * FROM `oper_coming` WHERE product_id = {$oneRow['product_id']}")
                ->queryAll();

            foreach($updateRow as $oneUpdateRow){
                $currentState = $calculateData[$oneUpdateRow['transaction_id']];
                
                expect($oneUpdateRow['amount'])->equals($currentState['amount'] - $oneUpdateRow['old_amount']);
                expect($oneUpdateRow['cost_price'])->equals($currentState['cost_price']);
                expect($oneUpdateRow['old_amount'])->equals($currentState['amount'] - $oneUpdateRow['amount']);
                expect($oneUpdateRow['old_cost_price'])->equals($currentState['old_cost_price']);
            }
        }
    ...
    {
        $resultatFilling  = $this->transactionHelper->fillTransaction($this->countTransaction,rand(1,5),'coming');
        expect($resultatFilling)->notEquals([]);
