<table id="transaction-table" class="table">
    <thead>
        ...
        <tr>
            <th colspan="2" style="text-align:center;font-size: 8px;padding: 1px">Цена прихода</th>
            <th colspan="2" style="text-align:center;font-size: 8px;padding: 1px">Цена прихода</th>
        </tr>
    </thead>
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td style="padding: 0 0 0 5px;font-size: 12px;border: 1px solid rgb(200,202,203)"><?= formatedPriceUSD($product[$i]->start_price); ?></td>
                <td style="padding: 0 0 0 5px;font-size: 12px;border: 1px solid rgb(200,202,203)"><?= formatedPriceUSD(getConvertUSDinUAH($product[$i]->start_price, $product[$i]->transaction->course)); ?></td>
                <td style="padding: 0 0 0 5px;font-size: 12px;border: 1px solid rgb(200,202,203)"><?= formatedPriceUSD($product[$i]->start_price * $product[$i]->amount); ?></td>
                <td style="padding: 0 0 0 5px;font-size: 12px;border: 1px solid rgb(200,202,203)"><?= formatedPriceUSD(getConvertUSDinUAH($product[$i]->start_price * $product[$i]->amount, $product[$i]->transaction->course)); ?></td>
            </tr>
        <?php endfor;?>
    </tbody>
</table>